...
<head>
    ...
    <title>ASMA'S ONLINE SERVICE CENTER</title>
    ...
    <style>
        ...
        * { 
        ...
        body.dark-mode .text-date-custom { color: #f8fafc !important; }
        body.dark-mode .text-time-custom { color: #cbd5e1 !important; opacity: 0.85; }

        body.light-mode { background: #f8fafc; color: #0f172a; }
        body.light-mode .card-bg { background: #ffffff; border: 1px solid #e2e8f0; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05); }
        body.light-mode .input-field { background: #ffffff; color: #0f172a; border: 1px solid #cbd5e1; }
        ...
        body.light-mode .text-date-custom { color: #1e293b !important; }
        body.light-mode .text-time-custom { color: #64748b !important; }
        ...
        .yellow-card-premium { 
            ...
            color: #000000; padding: 20px 16px; border-radius: 1.5rem;
            box-shadow: 0 10px 25px rgba(0, 0, 0, 0.1); 
        }

        .trans-table { width: 100%; border-collapse: collapse; table-layout: fixed; }
        .trans-table th { font-size: 11px; font-weight: 800; padding: 12px 4px; text-align: left; text-transform: uppercase; border-bottom: 2px solid rgba(0,0,0,0.05); }
        body.dark-mode .trans-table th { border-bottom-color: rgba(255,255,255,0.1); }
        .trans-table td { padding: 12px 4px; font-size: 13px; font-weight: 700; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
        
        .rem-info { font-size: 11px; font-weight: 600; display: block; overflow: hidden; text-overflow: ellipsis; margin-top: 2px; }
        body.dark-mode .rem-info { color: #facc15; }
        body.light-mode .rem-info { color: #475569; }
        
        .mode-badge { font-size: 9px; padding: 2px 4px; border-radius: 4px; background: rgba(0,0,0,0.06); font-weight: 800; margin-left: 4px; }
        body.dark-mode .mode-badge { background: rgba(255,255,255,0.1); }
...
<body class="light-mode h-full w-full flex flex-col overflow-hidden">
    ...
    <div class="w-full h-full flex flex-col overflow-hidden" id="mainApp" style="display:none;">
        ...
        <header class="header-custom sticky top-0 z-40 shadow-md w-full shrink-0">
            <div class="max-w-7xl mx-auto px-4 py-3 flex justify-between items-center">
                <div class="flex items-center gap-3 overflow-hidden">
                    ...
                    <h1 id="pageTitle" class="text-base font-black truncate max-w-[220px] uppercase tracking-wide">ASMA'S ONLINE SERVICE</h1>
                </div>
                <div class="flex gap-1.5 shrink-0">
                    <button id="custEditBtn" onclick="openCustModal(activeCid)" class="hidden icon-btn-box text-blue-400"><i class="fa-solid fa-user-pen"></i></button>
                    <button onclick="toggleTheme()" class="icon-btn-box"><i id="themeIcon" class="fa-solid fa-moon"></i></button>
                    <button onclick="lockApp()" class="icon-btn-box text-red-400"><i class="fa-solid fa-lock"></i></button>
                    <button onclick="openSettings()" class="icon-btn-box"><i class="fa-solid fa-gear"></i></button>
                </div>
            </div>
        </header>

        <div class="max-w-md mx-auto w-full flex-1 flex flex-col p-4 overflow-hidden relative">
            
            <div id="homeWorkspaceView" class="space-y-4 flex flex-col h-full w-full overflow-hidden">
                <div id="promiseHeader" class="space-y-2 overflow-y-auto max-h-[120px] custom-scroll shrink-0"></div>
                ...
                <div id="searchBox" class="shrink-0">
                    <div class="relative">
                        ...
                        <i class="fa-solid fa-magnifying-glass absolute right-3.5 top-4 opacity-40 text-main"></i>
                    </div>
                </div>
            ...
            <div id="ledgerWorkspaceView" class="hidden flex-col h-full w-full overflow-hidden relative">
                ...
                <div class="sticky top-0 z-30 pb-1 shrink-0 space-y-2">
                    <div id="topArea"></div>
                    
                    <div class="px-3 py-2 flex items-center justify-between gap-2 bg-slate-500/10 border border-slate-500/20 shrink-0 rounded-xl">
                        <div class="flex items-center gap-0.5 bg-black/20 dark:bg-black/40 rounded-lg p-0.5 text-xs font-bold">
                            <button id="typeFilterAll" onclick="setLedgerTypeFilter('ALL')" class="px-3 py-1 rounded-md bg-white dark:bg-slate-800 text-main shadow-xs">All</button>
                            <button id="typeFilterGave" onclick="setLedgerTypeFilter('GAVE')" class="px-3 py-1 rounded-md text-slate-400">Gave</button>
                            <button id="typeFilterGot" onclick="setLedgerTypeFilter('GOT')" class="px-3 py-1 rounded-md text-slate-400">Got</button>
                        </div>
                        
                        <div class="relative flex-1 max-w-[140px]">
                            <input type="text" id="ledgerSearchInput" oninput="renderDetails()" placeholder="Search..." class="w-full bg-black/10 dark:bg-black/30 border border-slate-500/20 rounded-lg py-1.5 pl-2 pr-6 text-xs font-bold outline-none text-main">
                            <i class="fa-solid fa-magnifying-glass absolute right-2 top-2.5 opacity-40 text-xs text-main"></i>
                        </div>
                    </div>

                    <div id="dateFilterDrawer" class="hidden px-3 py-2 bg-yellow-500/10 border border-yellow-500/20 flex items-center justify-between gap-2 text-xs rounded-xl">
                        <div class="flex items-center gap-1.5 text-main">
                            <input type="date" id="filterStartDate" onchange="renderDetails()" class="p-1 rounded bg-slate-200 dark:bg-white/10 outline-none max-w-[110px]">
                            <span class="font-bold">to</span>
                            <input type="date" id="filterEndDate" onchange="renderDetails()" class="p-1 rounded bg-slate-200 dark:bg-white/10 outline-none max-w-[110px]">
                        </div>
                        <button onclick="clearDateRangeFilters()" class="text-red-500 font-extrabold px-2 py-1 bg-red-500/5 rounded-md">Clear</button>
                    </div>

                    <div id="dailyRow" class="grid grid-cols-2 gap-3 py-0.5"></div>
                </div>

                <div class="flex-1 overflow-y-auto mt-1 custom-scroll w-full" style="margin-bottom: 60px;">
                    <table class="trans-table">
                        <thead class="sticky top-0 bg-[#f8fafc] dark:bg-[#1a3c30] z-10">
                            <tr class="text-slate-400 dark:text-slate-500">
                                <th style="width: 32%">Date</th>
                                <th style="width: 22%">Gave</th>
                                <th style="width: 22%">Got</th>
                                <th style="width: 24%">Balance</th>
                            </tr>
                        </thead>
                        <tbody id="ledgerItemsRowsContainer" class="divide-y divide-slate-500/10"></tbody>
                    </table>
                </div>

                <div id="footerActions" class="absolute bottom-0 left-0 right-0 p-3 bg-transparent backdrop-blur-md flex gap-4 z-40"></div>
            </div>
    ...
    <script>
        ...
        function renderPromises() {
            ...
            db.promises.forEach(p => {
                ...
                h += `<div class="p-3 rounded-xl flex justify-between items-center card-bg shadow-xs" onclick="openCustomer(${c.id})">
                    <div class="flex-1 text-left"><h3 class="font-bold text-xs sm:text-sm text-main">${c.name}: ₹${Number(p.amt).toFixed(2)}</h3><p class="text-[10px] opacity-60 text-main">Due: ${fmtDate(p.date)}</p></div>
                    <div class="flex gap-1.5">
                        <button onclick="event.stopPropagation();settlePromise(${p.id})" class="w-7 h-7 bg-green-600 rounded-lg flex items-center justify-center text-white text-xs"><i class="fa-solid fa-check"></i></button>
                        <button onclick="event.stopPropagation();deletePromise(${p.id})" class="w-7 h-7 bg-red-600 rounded-lg flex items-center justify-center text-white text-xs"><i class="fa-solid fa-trash-can"></i></button>
                    </div>
                </div>`;
            });
        ...
        function renderGlobalBalanceCard() {
            ...
            document.getElementById('globalBalanceCard').innerHTML = `
                <div class="yellow-card-premium text-center">
                    ...
                    <h2 class="text-4xl font-black mt-1">₹${Math.abs(netVal).toFixed(2)}</h2>
                </div>`;
        }
        ...
        function renderClientListFeed() {
            ...
            filtered.forEach(c => {
                ...
                html += `
                    <div class="flex items-center p-3.5 rounded-xl card-bg cursor-pointer transition shadow-xs" onclick="openCustomer(${c.id})">
                        ...
                        <div class="flex-1 ml-3 min-w-0">
                            <p class="font-bold text-sm truncate text-main">${c.name}</p>
                            <p class="text-[10px] text-slate-400 dark:text-slate-500 mt-0.5"><i class="fa-solid fa-phone text-[8px]"></i> ${c.phone || '-'}</p>
                        </div>
                        <div class="text-right shrink-0 ml-2">
                            <p class="font-black text-base ${bal>=0?'text-red-500':'text-green-500'}">₹${Math.abs(bal).toFixed(2)}</p>
                            <span class="text-[8px] font-bold opacity-50 uppercase text-main">${bal>=0?'Gave':'Got'}</span>
                        </div>
                    </div>`;
            });
        ...
        function setLedgerTypeFilter(filterVal) {
            ...
            ['All', 'Gave', 'Got'].forEach(lbl => {
                ...
                if(lbl.toUpperCase() === filterVal) {
                    btn.className = "px-3 py-1 rounded-md bg-white dark:bg-slate-800 text-main shadow-xs";
                } else {
                    btn.className = "px-3 py-1 rounded-md text-slate-400";
                }
            });
            renderDetails();
        }
        ...
        function renderDetails() {
            ...
            trans.forEach(t => {
                ...
                cumulativeBalance += (t.type === 'GAVE' ? t.amt : -t.amt);
                rb = cumulativeBalance; // Keep ultimate running balance updated
                ...
                const displayTime = t.time ? `<span class="text-[10px] font-bold block text-time-custom">${t.time}</span>` : '';

                const rowHtml = `
                <tr class="table-row-border hover:bg-slate-500/5 cursor-pointer transition" onclick="openEditTrans(${t.id})">
                    <td class="p-2 text-xs font-semibold break-words">
                        <div class="flex flex-col">
                            <div class="flex items-center">
                                <span class="font-extrabold text-sm text-date-custom">${fmtDate(t.date)}</span>
                                <span class="mode-badge ${t.mode==='UPI'?'text-blue-500':'text-emerald-600'}">${t.mode==='UPI'?'UPI':'C'}</span>
                            </div>
                            ${displayTime}
                            <span class="rem-info font-bold">${t.rem || ''}</span>
                        </div>
                    </td>
                    <td class="p-2 text-red-500 font-black text-sm sm:text-base truncate">₹${t.type==='GAVE'?t.amt.toFixed(0):'-'}</td>
                    <td class="p-2 text-green-500 font-black text-sm sm:text-base truncate">₹${t.type==='GOT'?t.amt.toFixed(0):'-'}</td>
                    <td class="p-2 ${cumulativeBalance>=0?'text-red-400':'text-green-400'} font-black text-sm sm:text-base truncate">₹${Math.abs(cumulativeBalance).toFixed(0)}</td>
                </tr>`;
                
                rowsArray.push(rowHtml);
            });

            // Reverse rows to show latest entry on top
            rowsArray.reverse();

            const netValText = rb >= 0 ? "BALANCE" : "ADVANCE";
            ...
            document.getElementById('topArea').innerHTML = `
                <div class="space-y-2">
                    ...
                    <div class="yellow-card-premium text-center">
                        <p class="text-[10px] font-black opacity-70 uppercase tracking-widest">${netValText}</p>
                        <h2 class="text-4xl font-black tracking-tight mt-0.5">₹${Math.abs(rb).toFixed(2)}</h2>
                        
                        <div class="grid grid-cols-3 gap-1 bg-black/5 dark:bg-white/10 rounded-xl p-2 mt-3 text-[10px] font-bold">
                            <div><p class="opacity-60">M-GAVE</p><p class="text-red-600 font-extrabold text-sm">₹${mGave.toFixed(0)}</p></div>
                            <div><p class="opacity-60">M-GOT</p><p class="text-green-700 font-extrabold text-sm">₹${mGot.toFixed(0)}</p></div>
                            <div><p class="opacity-60">M-NET</p><p class="${(mGave-mGot)>=0?'text-red-600':'text-green-700'} font-extrabold text-sm">₹${Math.abs(mGave-mGot).toFixed(0)}</p></div>
                        </div>
                    </div>
                </div>`;

            document.getElementById('dailyRow').innerHTML = `
                <div class="bg-emerald-500/5 border border-emerald-500/10 p-2 rounded-xl text-center"><span class="text-[10px] font-black opacity-60 text-main">CASH: </span><span class="text-green-500 font-black text-sm ml-1">₹${cToday.toFixed(0)}</span></div>
                <div class="bg-blue-500/5 border border-blue-500/10 p-2 rounded-xl text-center"><span class="text-[10px] font-black opacity-60 text-main">UPI: </span><span class="text-blue-500 font-black text-sm ml-1">₹${uToday.toFixed(0)}</span></div>`;

            document.getElementById('ledgerItemsRowsContainer').innerHTML = rowsArray.length ? rowsArray.join('') : `<tr><td colspan="4" class="text-center py-10 opacity-40 text-xs text-main">No records matched</td></tr>`;

            document.getElementById('footerActions').innerHTML = `
                <button class="flex-1 p-4 bg-red-600 hover:bg-red-700 rounded-2xl font-black text-white text-lg shadow-xl active:scale-95 transition-all" onclick="openAdd('GAVE')">GAVE</button>
                <button class="flex-1 p-4 bg-green-600 hover:bg-green-700 rounded-2xl font-black text-white text-lg shadow-xl active:scale-95 transition-all" onclick="openAdd('GOT')">GOT</button>`;
        }
        ...
        function goHome() { 
            ...
            document.getElementById('pageTitle').innerText = "ASMA'S ONLINE SERVICE";
            
            document.getElementById('ledgerWorkspaceView').classList.add('hidden');
            document.getElementById('ledgerWorkspaceView').classList.remove('flex');
            document.getElementById('homeWorkspaceView').classList.remove('hidden');
            
            renderClientListFeed();
        }

        // --- PROFILE CONTROL MODALS ---
        function openCustModal(cid=null) {
            ...
            openModal(`
                ...
                <div class="space-y-3 text-slate-800">
                    <input id="cn" class="w-full p-3.5 rounded-xl text-sm font-bold border border-slate-300 outline-none" placeholder="Client Name" value="${c.name}">
                    <input id="cp" type="number" class="w-full p-3.5 rounded-xl text-sm font-bold border border-slate-300 outline-none" placeholder="Phone Number" value="${c.phone}">
        ...
        // --- ACCOUNTING ENTRIES CONTROL ---
        function openAdd(type) {
            const timeStr = new Date().toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true });
            openModal(`
                <h2 class="text-base font-black mb-4 text-center uppercase tracking-wider">${type}</h2>
                <div class="space-y-4 text-slate-800">
                    <input id="ta" type="number" step="0.01" class="w-full p-4 mb-1 rounded-xl text-3xl text-center text-blue-600 font-black border border-slate-300 outline-none" placeholder="₹ 0" autofocus>
                    ...
                    <input id="tr" type="text" class="w-full p-4 rounded-xl text-md font-bold border border-slate-300 outline-none" placeholder="Remarks">
                    <div class="grid grid-cols-2 gap-2">
                        <input id="td" type="date" class="w-full p-4 rounded-xl text-md font-bold border border-slate-300 outline-none" value="${getToday()}">
                        <input id="tt" type="text" class="w-full p-4 rounded-xl text-md font-bold border border-slate-300 outline-none" value="${timeStr}">
                    </div>
        ...
        function openEditTrans(tid) {
            ...
            openModal(`
                ...
                <div class="space-y-3 text-slate-800">
                    <input id="eta" type="number" step="0.01" class="w-full p-4 rounded-xl text-3xl text-center text-blue-600 font-black border border-slate-300 outline-none" value="${t.amt}">
                    <input id="etr" type="text" class="w-full p-4 rounded-xl text-md font-bold border border-slate-300 outline-none" value="${t.rem || ''}" placeholder="Remarks">
                    <div class="grid grid-cols-2 gap-2">
                        <input id="etd" type="date" class="w-full p-4 rounded-xl text-md border border-slate-300 outline-none" value="${t.date}">
                        <input id="ett" type="text" class="w-full p-4 rounded-xl text-md border border-slate-300 outline-none" value="${t.time || ''}">
                    </div>
            ...
            openModal(`
                ...
                <div class="space-y-3.5 text-slate-800">
                    <div>
                        ...
                        <input id="upiI" class="w-full p-4 rounded-xl font-bold text-lg border border-slate-300 outline-none" value="${db.upi||''}" placeholder="UPI ID">
                    </div>
                    <div>
                        <label class="block text-[10px] text-slate-400 font-bold uppercase mb-1">PIN</label>
                        <input id="pinI" class="w-full p-4 rounded-xl font-bold text-lg border border-slate-300 outline-none" value="${db.pin}" placeholder="PIN">
                    </div>
        ...
        function resetAllData() { 
            if(confirm("മുന്നറിയിപ്പ്: നിങ്ങളുടെ എല്ലാ വിവരങ്ങളും എന്നെന്നേക്കുമായി ഡിലീറ്റ് ചെയ്യപ്പെടും. തുടരണോ?")) { 
        ...
        function openPromiseModal() { 
            openModal(`
                ...
                <div class="space-y-3 text-slate-800">
                    <input id="pa" type="number" step="0.01" class="w-full p-4 rounded-xl font-bold border border-slate-300 text-lg outline-none" placeholder="Amount">
                    <input id="pd" type="date" class="w-full p-4 rounded-xl font-bold border border-slate-300 text-lg outline-none" value="${getToday()}">
        ...
        function openPDFModal() {
            openModal(`
                <h2 class="mb-4 font-bold text-center uppercase text-slate-800">GENERATE REPORT</h2>
                <div class="space-y-4 text-slate-800">
                    <div class="grid grid-cols-2 gap-2">
                        <div>
                            <label class="block text-[10px] text-slate-400 font-bold mb-1">START DATE</label>
                            <input id="ps" type="date" class="w-full p-4 rounded-xl border border-slate-300 text-md font-bold outline-none" value="2026-01-01">
                        </div>
                        <div>
                            <label class="block text-[10px] text-slate-400 font-bold mb-1">END DATE</label>
                            <input id="pe" type="date" class="w-full p-4 rounded-xl border border-slate-300 text-md font-bold outline-none" value="${getToday()}">
                        </div>
                    </div>
        ...
        function generatePDF() {
            ...
            let rows=filtered.map(t=>{ 
                ...
                return `<tr><td style="border:1px solid #ddd;padding:8px">${fmtDate(t.date)}${tStr}</td><td style="border:1px solid #ddd;padding:8px;color:red">${t.type==='GAVE'?t.amt.toFixed(2):'-'}</td><td style="border:1px solid #ddd;padding:8px;color:green">${t.type==='GOT'?t.amt.toFixed(2):'-'}</td><td style="border:1px solid #ddd;padding:8px">${t.mode||'CASH'}</td><td style="border:1px solid #ddd;padding:8px;font-weight:bold">₹${rt.toFixed(2)}</td></tr>`; 
            }).join('');
            
            const finalStatusLabel = rt >= 0 ? "Total Outstanding Balance (ലഭിക്കാനുണ്ട്)" : "Total Advance Balance (നൽകാനുണ്ട്)";

            el.innerHTML=`
                <div style="font-family:sans-serif;">
                    <div style="text-align:center;margin-bottom:20px;">
                        <h2 style="margin:0;color:#0f172a;letter-spacing:1px;">ASMA'S ONLINE SERVICE CENTER</h2>
                        <p style="margin:4px 0;font-size:12px;color:#666;">Ledger Account Statement</p>
                    </div>
                    <div style="margin-bottom:15px;background:#f8fafc;padding:10px;border-radius:6px;border:1px solid #e2e8f0;">
                        <p style="margin:2px 0;"><strong>Client Name:</strong> ${cust.name}</p>
                        <p style="margin:2px 0;"><strong>Phone:</strong> ${cust.phone || '-'}</p>
                        <p style="margin:2px 0;font-size:10px;color:#55px;">Statement Period: ${fmtDate(s)} to ${fmtDate(e)}</p>
                    </div>
                    <table style="width:100%;border-collapse:collapse;margin-top:15px;text-align:left;">
                        <thead><tr style="background:#0f172a;color:white;"><th style="padding:8px;border:1px solid #0f172a;">Date</th><th style="padding:8px;border:1px solid #0f172a;">Gave (-)</th><th style="padding:8px;border:1px solid #0f172a;">Got (+)</th><th style="padding:8px;border:1px solid #0f172a;">Mode</th><th style="padding:8px;border:1px solid #0f172a;">Running Bal</th></tr></thead>
                        <tbody>${rows}</tbody>
                    </table>
                    <div style="margin-top:20px;padding:15px;background:#facc15;border-radius:8px;text-align:right;border:1px solid #eab308;">
                        <span style="font-size:11px;font-weight:bold;text-transform:uppercase;color:#000;display:block;opacity:0.8;">${finalStatusLabel}</span>
                        <span style="font-size:22px;font-weight:black;color:#000;margin-top:4px;display:block;">₹${Math.abs(rt).toFixed(2)}</span>
                    </div>
                </div>`;
                
            html2pdf().set({ margin: 0.5, filename: `${cust.name}_Statement.pdf`, image: { type: 'jpeg', quality: 0.98 }, html2canvas: { scale: 2 }, jsPDF: { unit: 'in', format: 'letter', orientation: 'portrait' } }).from(el).save();
        }
        ...
        function openQR(amt) {
            ...
            openModal(`
                <div class="bg-white p-5 rounded-3xl flex flex-col items-center text-slate-900">
                    ...
                    <p class="font-black text-3xl tracking-tight">₹${Number(amt).toFixed(2)}</p>
